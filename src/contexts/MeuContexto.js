...
export function MeuContextoProvider({ children }) {
  const feedItems = [
    {
      ...
      name: 'Saulo Tavares',
      drink: 'BLACK VELVET',
      description: 'Esse é o Black VelvetEsse é o Black VelvetEsse é o Black Velvet...',
    },
    {
      id: '2',
  ...
  const addPost = (photo, description) => {
    const newPost = {
      id: Date.now().toString(),
      image: photo,
      name: 'Nome do Autor',
      drink: 'Nome da Bebida',
      description,
      comments: [],
    };

    feedItems.unshift(newPost); // Adiciona o novo post no início do array
  };
